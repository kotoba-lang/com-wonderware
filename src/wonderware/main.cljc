(ns wonderware.main
  "Kotodama WASM entrypoint for the Wonderware clean-room actor (L4) — Clojure port.
  ...
  facts (`wonderware.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wonderware")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "wonderwa_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "wonderwa_con"
    :fields [:name :setpoint :processValue :mode]       :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}     :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "wonderwa_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"wonderware.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wonderware-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wonderware-compat/ready)
